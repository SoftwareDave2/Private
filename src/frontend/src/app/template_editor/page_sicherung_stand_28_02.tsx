...
const TemplateEditorPage: React.FC = () => {
    ...
    const handleSave = () => {
        ...
        // Lade das Hintergrundbild auf das Canvas, falls es noch nicht drauf ist
        ctx.clearRect(0, 0, canvas.width, canvas.height); // Canvas leeren
        if (bgImage) {
            ctx.drawImage(bgImage, 0, 0, canvas.width, canvas.height); // Bild wieder zeichnen
        }

        // Zeichne die Texte, die in den Eingabefeldern sind
        (templateConfig[selectedTemplate] || []).forEach(field => {
            const inputValue = inputValues[field.id] || ""; // Text aus dem Eingabefeld holen
            const x = field.xPercent * canvas.width;  // X-Position relativ zum Canvas
            const y = field.yPercent * canvas.height; // Y-Position relativ zum Canvas
            const width = field.widthPercent * canvas.width;
            const height = field.heightPercent * canvas.height;

            // Setze Text-Eigenschaften
            ctx.font = `${field.fontWeight} ${field.fontSize}`; // Schriftart und -größe
            ctx.textAlign = field.textAlign; // Textausrichtung
            ctx.textBaseline = 'middle'; // Vertikale Zentrierung des Textes
            ctx.fillStyle = field.color; // Textfarbe

            // Zeichne den Text auf das Canvas
            ctx.fillText(inputValue, x, y + height / 2); // Vertikal zentriert
        });
