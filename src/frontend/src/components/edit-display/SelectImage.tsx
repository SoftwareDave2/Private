...
export default function SelectImage({
    ...
    const safeOnUnselect = onUnselect ?? (() => {})
    ...
    useEffect(() => {
        ...
        if (selectedFilename) {
            ...
            if (selectedImgWidth > 0 && selectedImgHeight > 0 && !isResolutionMatch(selectedImgWidth, selectedImgHeight)) {
                safeOnUnselect()
            }
        }

        updateFilteredImages()
    }, [screenWidth, screenHeight, screenOrientation, selectedFilename])
    ...
    // Checks if an image roughly matches the display aspect ratio (±10%), respecting orientation.
    const isResolutionMatch = (imgWidth: number, imgHeight: number) => {
        if (!screenWidth || !screenHeight || imgHeight === 0 || imgWidth === 0) {
            return false
        }

        const normalizedScreenWidth = screenOrientation === 'horizontal' ? screenHeight : screenWidth
        const normalizedScreenHeight = screenOrientation === 'horizontal' ? screenWidth : screenHeight

        if (!normalizedScreenWidth || !normalizedScreenHeight) {
            return false
        }

        const tolerance = 0.1 // 10% Toleranz
        const screenRatio = normalizedScreenWidth / normalizedScreenHeight
    ...
    return (
        <>
            <div className="flex items-center gap-3">
                ...
                <Button type="button" variant="outlined" className="border-blue-gray-200 text-blue-gray-400" onClick={handleDialogOpen}>
                    Bild auswählen
                </Button>
            </div>
            <SelectImageDialog
